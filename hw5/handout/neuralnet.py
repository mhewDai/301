...
def random_init(shape):
    ...
    matrix = np.random.uniform(-0.1, 0.1, (M, D))
...
class SoftMaxCrossEntropy:
    ...
    def _cross_entropy(self, y: int, y_hat: np.ndarray) -> float:
        ...
        """
        y_arr = zero_init(y_hat.shape[0])  
        y_arr[y] = 1 
        return -np.sum(np.dot(y_arr, np.log(y_hat)))
...
class Linear:
    def __init__(self, input_size: int, output_size: int,
        ...
        self.dw = zero_init(self.w.shape)
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        # TODO: perform forward pass and save any values you may need for
        #  the backward pass
        x_bias = np.concatenate((np.array([1]), x))
        self.x_bias = x_bias
        z = self.w @ x_bias
        return z
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO: implement
        self.dw = np.outer(dz, self.x_bias)
...
class NN:
    ...
    def forward(self, x: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        a2 = self.sigmoid.forward(a1)
        a3 = self.linear2.forward(a2)
        y_hat, loss = self.softmax.forward(a3, y)
        return y_hat, loss
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> None:
        ...
        dz = self.softmax.backward(y, y_hat)
        dz = self.linear2.backward(dz)
        dz = self.sigmoid.backward(dz)
    ...
    def train(self, X_tr: np.ndarray, y_tr: np.ndarray,
        ...
        for epoch in range(n_epochs):
            ...
            for i in range(len(y_tr)):
                y_hat, _ = self.forward(X_tr_shuffled[i], y_tr_shuffled[i])
            ...
            print(f"Epoch {epoch+1}, Train Loss: {train_loss}, Validation Loss: {test_loss}")
    ...
    def test(self, X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        error_rate = 1 - (correct_predictions / len(y))
